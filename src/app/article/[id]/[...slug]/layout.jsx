        ...
        {currentArticle && (
          <div className="relative mb-12 h-[67vh] w-full overflow-hidden rounded-xl">
            ...
            <div className="absolute inset-0 flex items-end justify-start rounded-xl bg-g4c bg-opacity-[50%]">
              <h4 className="absolute left-1/2 top-1/2 w-full -translate-x-1/2 -translate-y-1/2 text-center text-[1.7rem] text-gf lg:text-[2.8125vw] font-Holispay">{currentArticle.title}</h4>
              <div className="flex items-center justify-start gap-1 p-5 lg:p-7">
                <span className="text-base text-gDarkYellow lg:text-[1.25vw] font-PoppinsRegular">{currentArticle.publishedAt}</span>
                <span className="text-base text-gf lg:text-[1.25vw] font-PoppinsRegular">/</span>
                <span className="text-base text-gf lg:text-[1.25vw] font-PoppinsRegular">{currentArticle.readingMinutes} minutes</span>
              </div>
            </div>
          </div>
        )}
        <div className="hidden w-full rounded-xl border-[1.5px] border-ge4 bg-gec p-5 lg:block">
          <h4 className="w-full border-b-[1px] border-gd5 pb-3 text-[1.7rem] text-g4c lg:pb-[0.78125vw] lg:text-[2.3vw] font-Holispay">Latest Articles</h4>
          <ul className="mt-4 w-full lg:mt-[16px]">
            {data.map((item) => (
              <li key={item.id}>
                <Link href={`/article/${item.id}/${item.slug}`} className="flex items-start justify-start gap-2 py-2 text-base text-g4c lg:gap-[0.5208333333333334vw] lg:py-[0.5208333333333334vw] lg:text-[1.15vw] font-PoppinsRegular">
