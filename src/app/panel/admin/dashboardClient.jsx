    ...
    <main dir="rtl" className="min-h-screen bg-gray-100 p-4 text-gray-900 md:p-8">
      <div className="mx-auto max-w-6xl">
        <header className="mb-6 flex flex-col gap-4 rounded-2xl bg-gray-900 p-5 text-white sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold">مدیریت محتوای گندم</h1>
        ...
        <nav className="mb-6 flex flex-wrap gap-2 rounded-2xl bg-white p-2 shadow-sm">
          ...
          <button onClick={() => setTab("articles")} className={`rounded-xl px-5 py-2.5 text-sm ${tab === "articles" ? "bg-gray-900 text-white" : "text-gray-600"}`}>مقالات</button>
          <button onClick={() => setTab("content")} className={`rounded-xl px-5 py-2.5 text-sm ${tab === "content" ? "bg-gray-900 text-white" : "text-gray-600"}`}>محتوای سایت</button>
        </nav>
        ...
        {tab === "messages" ? (
         ...
         ) : tab === "team" ? <TeamManager /> : tab === "articles" ? <ArticleManager /> : <SiteContentManager />}
      </div>
    </main>
  )
}
