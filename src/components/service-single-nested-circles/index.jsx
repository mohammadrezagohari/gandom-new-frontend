    ...
    <div className="relative w-[46.875vw] h-[46.875vw] rounded-full border-[1.59px] border-[#FFD10126]  flex items-center justify-center">
      ...
      <ul className="tech socialIconDownSmUl  text-center p-2 absolute left-0  flex justify-center items-center gap-7">
        {technologiesItems.slice(0, 5).map((tec, i) => (
          <li
            key={i}
            // style={{ left: tec.left, top: tec.top }}
            className={`left-item-${i} bg-transparent text-gf text-2xl w-[5vw] h-[5vw] border-[2.39px] border-[#D9B66A]  flex justify-center items-center rounded-full hover:cursor-pointer `}
          >
            {tec.icon}
            <span className="">{tec.title}</span>
          </li>
        ))}
      </ul>
      ...
      <ul className="tech socialIconUpSmUl text-center p-2 absolute right-0   flex justify-center items-center gap-7">
        {technologiesItems.slice(5, 10).map((tec, i) => (
          <li
            key={i}
            // style={{ left: tec.left, top: tec.top }}
            className={`right-item-${i}  bg-transparent text-gf text-2xl w-[5vw] h-[5vw] border-[2.39px] border-[#D9B66A]  flex justify-center items-center rounded-full hover:cursor-pointer `}
          >
            {tec.icon}
            <span
              className=""
