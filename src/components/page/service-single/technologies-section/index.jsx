...
import NestedCircles from "@/src/components/service-single-nested-circles";
...
function TechnologiesSection() {
  const technologiesItems = [
    { id: 1, icon: <SiDotnet />, title: "Core" },
    { id: 2, icon: <SiMicrosoftsqlserver />, title: "SQL" },
    { id: 3, icon: <SiMongodb />, title: "Mongodb" },

    { id: 4, icon: <SiNextdotjs />, title: "Next" },
    {
      id: 5,
      icon: <SiNodedotjs />,
      title: "NodeJs",
    ...
    { id: 8, icon: <SiRedis />, title: "Redis" },
    { id: 9, icon: <SiLaravel />, title: "Laravel" },
    {
      id: 10,
      icon: <SiAngularjs />,
      title: "Angular",
        ...
        <div className="lg:hidden  flex items-center justify-center divide-x">
          ...
          <ul className="transition ease-in-out delay-150 flex flex-col justify-start gap-2">
            {technologiesItems.slice(0, 5).map((item, i) => (
              <li
                key={i}
                className="p-2 mr-3 technologiesItems text-gf hover:text-g21 rounded-xl hover:bg-gradient-to-r from-[#FFFFFFF2] to-[#FFFFFF4D] flex items-center justify-end gap-3 relative hover:cursor-pointer"
              >
                <span className=" text-xl">{item.title}</span>
                <div className="bg-transparent text-2xl w-[44px] h-[44px] flex justify-center items-center rounded-full">
                  {item.icon}
                </div>
              </li>
            ))}
          ...
          <ul className="transition ease-in-out delay-150 flex flex-col justify-end gap-2">
            {technologiesItems.slice(5, 10).map((item, i) => (
              <li
                key={i}
                className="p-2 ml-3 technologiesItems text-gf hover:text-g21 rounded-xl hover:bg-gradient-to-r from-[#FFFFFF4D] to-[#FFFFFFF2] flex items-center justify-start gap-3 relative hover:cursor-pointer"
              >
                <div className="bg-transparent text-2xl w-[44px] h-[44px] flex justify-center items-center rounded-full">
                  {item.icon}
                </div>
